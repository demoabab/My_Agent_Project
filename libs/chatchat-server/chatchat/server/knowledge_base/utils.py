...
def _get_tenant_path_prefix(tenant_id: str = None) -> str:
    """获取租户路径前缀。当 tenant_id 为 None 时尝试从 contextvars 读取。"""
    if tenant_id is None:
        try:
            ...
            pass
    if tenant_id:
        return os.path.join(Settings.basic_settings.KB_ROOT_PATH, tenant_id)
    return Settings.basic_settings.KB_ROOT_PATH


def get_kb_path(knowledge_base_name: str, tenant_id: str = None):
    return os.path.join(_get_tenant_path_prefix(tenant_id), knowledge_base_name)


def get_doc_path(knowledge_base_name: str, tenant_id: str = None):
    return os.path.join(get_kb_path(knowledge_base_name, tenant_id), "content")


def get_vs_path(knowledge_base_name: str, vector_name: str, tenant_id: str = None):
    return os.path.join(get_kb_path(knowledge_base_name, tenant_id), "vector_store", vector_name)


def get_file_path(knowledge_base_name: str, doc_name: str, tenant_id: str = None):
    doc_path = Path(get_doc_path(knowledge_base_name, tenant_id)).resolve()
    file_path = (doc_path / doc_name).resolve()
    if str(file_path).startswith(str(doc_path)):
        return str(file_path)


def list_kbs_from_folder(tenant_id: str = None):
    root = _get_tenant_path_prefix(tenant_id)
...
def list_files_from_folder(kb_name: str, tenant_id: str = None):
    doc_path = get_doc_path(kb_name, tenant_id)
    result = []
