...
def search_docs(
    ...
    kb = KBServiceFactory.get_service_by_name(knowledge_base_name, tenant_id=current_user.get("tenant_id") if isinstance(current_user, dict) else None)
    data = []
    if kb is not None:
        if query:
...
def list_files(
    ...
    if kb is None:
        ...
        all_docs = get_kb_file_details(knowledge_base_name, tenant_id=tenant_id)
        return ListResponse(data=all_docs)


def _save_files_in_thread(
        files: List[UploadFile], knowledge_base_name: str, override: bool, tenant_id: str = None
):
    """
    ...
    def save_file(file: UploadFile, knowledge_base_name: str, override: bool, tenant_id: str = None) -> dict:
        """
        保存单个文件。
        """
        try:
            ...
            file_path = get_file_path(
                knowledge_base_name=knowledge_base_name, doc_name=filename, tenant_id=tenant_id
            )
    ...
    params = [
        {"file": file, "knowledge_base_name": knowledge_base_name, "override": override, "tenant_id": tenant_id}
        for file in files
    ]
...
def upload_docs(
    ...
    for result in _save_files_in_thread(
            files, knowledge_base_name=knowledge_base_name, override=override, tenant_id=tenant_id
    ):
    ...
    for file_name in file_names:
        ...
        try:
            kb_file = KnowledgeFile(
                filename=file_name, knowledge_base_name=knowledge_base_name, tenant_id=tenant_id
            )
...
def update_docs(
    ...
    # 生成需要加载docs的文件列表
    for file_name in file_names:
        file_detail = get_file_detail(kb_name=knowledge_base_name, filename=file_name, tenant_id=tenant_id)
...
def recreate_vector_store(
    ...
    def output():
        try:
            ...
            kb = KBServiceFactory.get_service_by_name(knowledge_base_name, tenant_id=_tid)
            if kb is None:
                ...
                if not ok:
                    ...
                    for status, result in files2docs_in_thread(
                        ...
                        if status:
                            ...
                            kb_file = KnowledgeFile(
                                filename=file_name, knowledge_base_name=kb_name, tenant_id=_tid
                            )
