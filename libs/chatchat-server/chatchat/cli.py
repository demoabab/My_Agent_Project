...
@main.group("db", help="数据库迁移管理")
def db_group():
    """Alembic 数据库迁移命令组"""
    pass


@db_group.command("upgrade", help="升级数据库到最新版本")
@click.option("--revision", default="head", help="目标版本 (默认: head)")
def db_upgrade(revision: str):
    _run_alembic_command("upgrade", revision)


@db_group.command("downgrade", help="回滚数据库到指定版本")
@click.option("--revision", required=True, help="目标版本")
def db_downgrade(revision: str):
    _run_alembic_command("downgrade", revision)


@db_group.command("history", help="查看迁移历史")
def db_history():
    _run_alembic_command("history", "")


@db_group.command("current", help="查看当前数据库版本")
def db_current():
    _run_alembic_command("current", "")


def _run_alembic_command(cmd: str, revision: str):
    """Helper to run Alembic commands with proper config."""
    try:
        from alembic import command
        from alembic.config import Config
    except ImportError:
        logger.error("alembic 未安装，请运行: poetry add alembic")
        return
    ...
    try:
        if cmd == "upgrade":
            ...
            logger.success(f"数据库已升级到: {revision}")
        elif cmd == "downgrade":
            command.downgrade(alembic_cfg, revision)
            logger.success(f"数据库已回滚到: {revision}")
        elif cmd == "history":
