...
def run_api_server(
    ...
    from chatchat.server.api_server.server_app import AppConfig, create_app
    ...
    set_httpx_config()
    app_config = AppConfig(run_mode=run_mode)
...
def main(all, api, webui):
    ...
    # 运行数据库迁移
    from chatchat.settings import validate_settings

    warnings = validate_settings()
    for w in warnings:
        logger.warning(w)

    _run_migrations_or_create_tables()
    _seed_default_data()
...
def _run_migrations_or_create_tables():
    """运行 Alembic 迁移，若不可用则回退到 create_all。"""
    ...
    try:
        ...
        if alembic_ini.exists():
            ...
            _fallback_create_tables()
    except ImportError:
        _fallback_create_tables()


def _fallback_create_tables():
    from chatchat.server.db.manager import db_manager
    db_manager.create_all_tables()


def _seed_default_data():
    """幂等初始化默认管理员用户、默认租户和基础权限。"""
    from chatchat.server.db.seed import seed_all
    seed_all()
