...
class BasicSettings(BaseFileSettings):
    ...
    """是否启用配置文件自动重载。生产环境建议关闭。"""

    JWT_SECRET_KEY: str = "chatchat-secret-key-change-in-production"
    """JWT 签名密钥。生产环境请务必修改为随机字符串。"""
...
class PlatformConfig(MyBaseModel):
    ...
    """api key if available. 支持 ${ENV_VAR} 格式引用环境变量"""
    ...
    def resolve_env_vars(self):
        """解析 api_key, api_base_url, api_proxy 中的 ${VAR} 环境变量引用"""
        import re
        for field_name in ("api_key", "api_base_url", "api_proxy"):
...
def validate_settings() -> list[str]:
    """启动时验证关键配置。返回告警消息列表。致命问题抛出 ConfigurationError。"""
    from chatchat.env import get_environment
    ...
    if uri.startswith("sqlite"):
        if env == "prod":
            raise ConfigurationError("SQLite is not allowed in production environment")
        warnings.append("Using SQLite - suitable for dev only")

    # KB_ROOT_PATH 存在性检查
    kb_path = Path(bs.KB_ROOT_PATH)
    if not kb_path.exists():
        warnings.append(f"KB_ROOT_PATH does not exist: {kb_path}")

    # 生产环境检查 API Key 不是默认值
    if env == "prod":
        for platform in Settings.model_settings.MODEL_PLATFORMS:
            key = platform.api_key.get_secret_value()
            if key in ("EMPTY", ""):
                warnings.append(
                    f"Platform '{platform.platform_name}' has empty API key"
                )
